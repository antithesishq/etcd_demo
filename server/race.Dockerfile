...
# The instrumentor may downgrade go.work version; fix it to match the current Go toolchain
RUN go work use . server etcdutl etcdctl
RUN CGO_ENABLED=1 GO_BUILD_FLAGS=-race make build
CMD ["./bin/etcd"]
